"""La formattazione e' l'unica cosa che l'utente legge in ogni numero."""

from setxray import fmt


def test_separatori_inglesi():
...
def test_importi_grandi():
    ...
    assert fmt.big(1.5e12) == "1,500.0bn THB"
    assert fmt.big(4.2e6) == "4.2m THB"
    assert fmt.big(-98.3e9) == "-98.3bn THB"


def test_valori_mancanti_sempre_dichiarati():
    for funzione in (fmt.num, fmt.pct, fmt.mult, fmt.money, fmt.big, fmt.ratio, fmt.date):
        assert funzione(None) == "n/a"
    assert fmt.num(float("nan")) == "n/a" or fmt.num(float("nan")) == "nan"
